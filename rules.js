...
function begin_play(){
	...
	if(game_started() == true){
		alert("Game is in progress, please select 'Reset Play' to start a new game.");
		return false;
	}

	if(PlayerOne != "" && PlayerOne==PlayerTwo){ //non empty same name
		alert("Players cannot have the same name, someone must edit their name to continue");
		return false;
	}

	if(isEmpty(PlayerOne) == false && isEmpty(PlayerTwo) == false){

		started = true;
		document.getElementById("player1_id").value += " is X"; //Player one will always be x
		document.getElementById("player2_id").value += " is O"; //Player two will always be y

		document.getElementById("player1_id").disabled = true;
		document.getElementById("player2_id").disabled = true;

		document.getElementById('turn_info').innerHTML = "Player One Goes First";
	}
	else{
		alert("Please enter both name fields");
		return false;
	}
}
...
function reset_play(){
	...
	var i = 0;
	for(i; i < 9; i++){
		document.getElementById(table_ids[i]).innerHTML = table_ids[i]; //clear board
	}

	started = false; //new game
	turn = 1; //refresh turns

	document.getElementById("turn_info").innerHTML = "Game has not begun."; //change display
	document.getElementById("move_text_id").value = "";
	document.getElementById("player1_id").value = ""; //set player 1 name back to empty string
	...
	board_state = [-1, -1, -1, -1, -1, -1, -1, -1, -1]; //reset board_state
}
		...
		if(Valid){
			...
			if(whose_move()){
				if(board_state[indexInput] == -1){
					...
					toggle_move();
					document.getElementById("turn_info").innerHTML = "Turn for: <strong> 0 </strong>";
				}
				else{
					alert("This position is already filled! Please choose an empty location.")
				}
			}
			else{
				if(board_state[indexInput] == -1){
...
function endGame(){
	var gameOver = (document.getElementById("A1").innerHTML == document.getElementById("A2").innerHTML //win horizontally
